...
export const FOOTER = ({ lang, isEdit, updateAt, pw, vpw, mode, share, shareId, path, views, theme, sharePath }) => `
    <div class="footer">
        ...
        <div class="footer-controls">
            <button id="present-btn" class="opt-button" style="background:#673ab7;color:white;padding:4px 10px;border-radius:4px;font-weight:500;display:flex;align-items:center;gap:4px;" title="進入全螢幕簡報模式">📽️ Present</button>
            <label class="footer-label" for="preview-width-selector">Width</label>
            ...
            <select id="theme-selector" class="footer-select">
                ${Object.keys(THEMES).map(t => `<option value="${t}" ${t === (theme || 'catppuccin-macchiato') ? 'selected' : ''}>${t}</option>`).join('')}
            </select>
            ...
            ${views ? `<span class="views-count">👁 ${views} views</span>` : ''}
