...
The interoperable core accepts `text`, `file`, `link`, `group`, and edge records. DAVID888 sticky-note appearance is stored in an optional `david888` extension object, which other JSON Canvas tools can safely ignore. In the browser, selected nodes expose color, duplicate, and delete controls; selected edges expose label, arrow direction, line-style, width, color, and delete controls. Use `/new/canvas` to create one; use its Canvas toolbar to import or export `.canvas` files. The Footer `Import content` menu remains available for Markdown, audio, URL, Office, and PDF workflows; from Canvas those actions follow the existing Markdown-note import behavior and do not mutate the Canvas JSON.
