...
The interoperable core accepts `text`, `file`, `link`, `group`, and edge records. DAVID888 sticky-note appearance is stored in an optional `david888` extension object, which other JSON Canvas tools can safely ignore. In the browser, all card types share React Flow UI-style `BaseNode` anatomy and visible four-sided `BaseHandle` points; selected nodes expose color, duplicate, delete, and Undo/Redo controls; selected edges expose the native `EdgeToolbar` for label, arrow direction, line-style, width, color, and delete controls. Relationship edges render visible SVG strokes and forward React Flow marker URLs as strings, so arrowheads do not become invalid `[object Object]` SVG attributes; the edge layer and SVG drawing surface are explicitly sized to the flow viewport to prevent a `0px` edge canvas. Canvas supports box selection, multi-drag, bulk color/delete, Cmd/Ctrl+F search, SVG/PNG export, WikiLink graph generation, and Camera Tour playback. On narrow viewports, fit view uses a `0.85x` minimum initial zoom so card labels remain readable while the canvas remains pannable. Unpublished `draft`/`pending` Canvas records are restored from the local cache after reload and the viewport refits after node measurement; unpublished drafts remain local until publishing or published-note autosave syncs them to the cloud. Canvas assets use cache-busted `?v=3.8` URLs. Use `/new/canvas` to create one; use its Canvas toolbar to import or export `.canvas` files. The Canvas surface follows the selected light/dark UI theme and its starter template follows the page language. The Footer `Import content` menu remains available for Markdown, audio, URL, Office, and PDF workflows; from Canvas those actions follow the existing Markdown-note import behavior and do not mutate the Canvas JSON.
...
The native MCP endpoint exposes `validate_canvas`, `write_canvas`, `read_canvas`, and `generate_canvas_from_wikilinks`. Use these tools when a user asks for a visual relationship map, architecture canvas, concept graph, connected card diagram, or a graph generated from Markdown `[[WikiLink]]` references. Return the generated `shareUrl` so the human can open the Canvas.
...
Canvas interaction capabilities include box selection and multi-drag, bulk color/delete actions, Cmd/Ctrl+F card search, SVG/PNG export, WikiLink graph generation from a Markdown path, and Camera Tour playback. Agents may use these features when a visual relationship map or navigable presentation improves the result.
