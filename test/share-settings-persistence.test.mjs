...
test('setting route persists width, share font, and preview device metadata', () => {
    assert.match(indexSource, /const\s+\{\s*mode,\s*share,\s*theme,\s*width,\s*shareFont,\s*previewDevice\s*\}\s*=\s*await request\.json\(\)/)
