...
### 4. AI 輔助寫作與編輯
- **AI 格式化排版**：內建由 Cloudflare Workers AI（`gpt-oss-20b`）驅動的「AI排版」按鈕，可一鍵優化 Markdown 排版、修正語法與標點符號，使文件更加專業。
- **AI 輔助編輯與產生**：整合「AI編輯」按鈕，基於 `gpt-oss-120b` 模型，支援指令式的段落插入、部分改寫或整篇筆記內容潤色與生成。
- **選取文字 AI 捷徑**：在編輯器中選取任意文字時，會自動彈出「AI編輯」捷徑按鈕，可僅針對選取的特定區塊進行指令式 AI 修改、校對與生成，其餘未選取的內容保持不變，極具效率。
...
### 4. AI-Assisted Writing & Editing
- **AI Document Formatting**: Built-in "Format" button powered by Cloudflare Workers AI (`gpt-oss-20b`) to instantly clean up Markdown formatting, correct typos, and standardize punctuation with one click.
- **AI-Assisted Editing & Generation**: Integrated "AI Edit" button leveraging the `gpt-oss-120b` model, allowing instruction-based section insertion, partial rewriting, or full-note copy refinement and content generation.
- **Contextual Selection AI Shortcut**: Selecting any text range inside the editor triggers a floating "AI Edit" shortcut. Instruct the AI to modify or translate only the highlighted selection while keeping the rest of your note completely untouched.

### 5. Presentation, Print & Analytics
