...
- **可安裝 PWA 與安全離線提示**：在支援的 Android Chromium 瀏覽器中，當瀏覽器判定可安裝時，頁面底部會出現「安裝 App」；點擊後開啟原生安裝確認。已安裝的獨立 App 不會再顯示此提示，點擊 `×` 後在該分頁也不會重現。也可從瀏覽器選單選擇「安裝 App」或「加入主畫面」，以獨立視窗開啟。離線時導覽會顯示內建提示頁；目前不快取筆記、Share 頁或 API 回應，因此不支援離線閱讀／編輯，也不會將受保護內容存入瀏覽器快取。
...
- **Installable PWA with Safe Offline Handling**: In supported Android Chromium browsers, an in-page Install app action appears after the browser reports the PWA is installable; clicking it opens the native install confirmation. Installed standalone windows never show this promotion, and dismissing it with `×` keeps it hidden for the current tab. You can also choose Install app or Add to Home Screen from the browser menu to launch the notepad in a standalone window. Offline navigation shows a built-in fallback page. Notes, Share pages, and API responses are deliberately not cached, so offline reading/editing is not available and protected content is never stored in browser cache.
