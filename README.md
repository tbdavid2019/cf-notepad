...
- **🎨 Canvas v2 畫布整體架構 (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。全面升級為 Canvas v2 架構，借鑑成熟開源工具專案 **Ameliorate**（`0cacee5577438979b651dd808793c4cbd13864ee`，MIT License，版權致謝於 `THIRD_PARTY_NOTICES.md`）：
  - **嚴格 JSON Canvas 1.0 資料邊界**：底層儲存全面相容 JSON Canvas 規範（標準 `text`、`file`、`link`、`group` 節點與 `edges`），透過 `model/jsonCanvasAdapter.mjs` 確保 100% round-trip 保真。
  - **Zustand 交易式狀態管理與 Undo/Redo**：指令（建立、拖曳、縮放、換色、複製、連線、樣式切換）均產生單一交易歷史紀錄，支援 Cmd+Z / Cmd+Shift+Z 復原與重做。
  - **模組化節點系統 (`FlowNode`)**：節點外殼共用四邊 Loose handles、縮放邊框與選取狀態，專門子組件負責 `MarkdownNode`、`StickyNode`、`WikiNode`、`LinkNode` 與 `GroupNode`；選取時呈現精緻的浮動動作工具列 `NodeToolbar`。
  - **靈活關係線與樣式面板 (`CanvasEdge` &amp; `EdgeToolbar`)**：Smoothstep 貝茲曲線與 20px 隱形感應軌道，支援標籤文字原地編輯，選取時提供箭頭方向（無、起點、終點、雙向）、線條樣式（實線、虛線、點線）、粗細（1.5、2.5、4px）與顏色即時切換。
  - **置底工具列與檔案操作 (`MainToolbar`)**：底部緊湊工具列整合新增卡片選單、復原/重做、視圖置中 (`fitView`) 與 `.canvas` 檔案匯入/匯出/清空功能。
  - **獨立隔離樣式 (Isolated Stylesheet)**：樣式完全獨立於 `static/js/canvas-v2/canvas-v2.css` 與語意化 `tokens.css`，零污染核心筆記佈局，完美支援 20 款主題與深淺色模式切換。唯讀分享模式鎖定座標防誤觸，同時保留平移與縮放。
...
- **🎨 Canvas v2 Architecture (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. Fully upgraded to Canvas v2 based on the open-source **Ameliorate** project (`0cacee5577438979b651dd808793c4cbd13864ee`, MIT License, credited in `THIRD_PARTY_NOTICES.md`):
  - **Strict JSON Canvas 1.0 Boundary**: Fully adheres to the JSON Canvas specification (`text`, `file`, `link`, `group` nodes and `edges`) via `model/jsonCanvasAdapter.mjs` ensuring 100% round-trip fidelity.
  - **Zustand Command Architecture &amp; Undo/Redo**: Mutations (create, drag, resize, recolor, duplicate, connect, style changes) execute as single-step transactions with native Cmd+Z / Cmd+Shift+Z Undo and Redo.
  - **Modular Node System (`FlowNode`)**: Shared outer shell with four-sided Loose handles, resize boundaries, and selection indicators, with dedicated content components for `MarkdownNode`, `StickyNode`, `WikiNode`, `LinkNode`, and `GroupNode`, accompanied by a contextual `NodeToolbar`.
  - **Smoothstep Edges &amp; Style Popover (`CanvasEdge` &amp; `EdgeToolbar`)**: Smoothstep curves with 20px hit detection, inline label editing, and a popover panel configuring arrow directions (none, start, end, both), line styles (solid, dashed, dotted), stroke widths (1.5, 2.5, 4px), and colors.
  - **Compact Bottom Toolbar (`MainToolbar`)**: Pinned bottom center toolbar featuring an Add menu, Undo/Redo, Fit view, and File menu (`.canvas` import, export, and clear canvas).
  - **Isolated Stylesheet**: Styled exclusively through `static/js/canvas-v2/canvas-v2.css` and semantic `tokens.css`, preventing style bleed into or from core notepad templates while providing complete dark and light mode coverage. Read-only sharing mode locks node coordinates while preserving pan and zoom.
