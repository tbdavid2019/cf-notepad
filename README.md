...
- **🎨 Canvas 畫布與卡片連線 (Canvas with JSON Canvas &amp; React Flow)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。資料以 JSON Canvas 1.0 核心儲存，支援標準 text、file、link、group 節點與 edges；卡片共用 React Flow UI-style `BaseNode` anatomy，連線點共用可見 `BaseHandle`，並使用原生 `NodeToolbar`／`EdgeToolbar`。畫布互動採 React Flow Feature Overview 的水平 Controls、dotted Background、MiniMap、snap-to-grid、fitView 與 infinite zoom 基準。支援實心舒適的 6 色調色盤與原生自訂色彩填色、便籤／連結／群組換色、一鍵複製卡片、四邊連線、關係文字原地編輯、獨立箭頭方向／線條樣式／線寬／顏色設定，以及 Canvas Undo／Redo。嚴格統一卡片字體排版，編輯框與預覽具備一致字級行高。本機未發布草稿會在重新整理後恢復，發布前會保存目前完整畫布，並具備安全降級容錯；Canvas 也支援深色模式與中英文預設模板。Canvas 工具列可匯入／匯出 `.canvas` 檔案。Footer 的「匯入內容」仍保留 Word、PDF、Office、音訊、網址與 Markdown 等既有功能。唯讀分享模式鎖定座標，同時保留平移（Pan）與縮放（Zoom）。
...
- **🎨 Canvas with JSON Canvas &amp; React Flow**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. It persists the JSON Canvas 1.0 core—standard text, file, link, group, and edge records—with shared React Flow UI-style `BaseNode` anatomy, visible four-sided `BaseHandle` points, and native `NodeToolbar`/`EdgeToolbar` patterns. It provides solid 6-color card fills, native custom color selection, recoloring for cards/sticky notes/links/groups, duplicate/delete actions, edge label editing, explicit arrow direction, line style, width, and color controls, plus Undo/Redo. Typography stays consistent between preview and editing modes. Canvas also restores local drafts after reload, persists the current document at publish time with relaxed fallback recovery, supports dark mode and bilingual starter templates, and imports/exports `.canvas` files. The Footer `Import content` menu remains available for Word, PDF, Office, audio, URL, and Markdown workflows. Read-only sharing keeps pan and zoom while locking edits.
