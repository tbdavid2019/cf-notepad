...
- **🎨 Canvas 畫布與卡片連線 (Canvas with JSON Canvas &amp; React Flow)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。資料以 JSON Canvas 1.0 核心儲存，支援標準 text、file、link、group 節點與 edges；卡片採用極簡零擬窗（Zero-Chrome）無多餘標題列設計，100% 空間保留給正文，並具備卡片浮動工具列 (`NodeToolbar`) 與關係線浮動工具列 (`EdgeToolbar`)。支援實心舒適的 6 色調色盤與原生自訂色彩填色、便籤／連結／群組換色、一鍵複製卡片、卡片右上角懸停無佔位動作按鈕、四邊連線錨點智能淡入、關係文字原地編輯、獨立箭頭方向／線條樣式／線寬／顏色設定，以及 Canvas Undo／Redo。嚴格統一遍歷卡片字體排版，編輯框與預覽具備完全一致的字級行高，杜絕字體忽大忽小與非必要捲軸。本機未發布草稿會在重新整理後恢復，發布前會保存目前完整畫布，並具備安全降級容錯；Canvas 也支援深色模式與中英文預設模板。Canvas 工具列可匯入／匯出 `.canvas` 檔案。Footer 的「匯入內容」仍保留 Word、PDF、Office、音訊、網址與 Markdown 等既有功能。唯讀分享模式鎖定座標，同時保留平移（Pan）與縮放（Zoom）。
...
- **🎨 Canvas with JSON Canvas &amp; React Flow**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. It persists the JSON Canvas 1.0 core—standard text, file, link, group, and edge records—with zero-chrome cards eliminating fake window title bars to preserve 100% space for note content. Features floating `NodeToolbar` (rich solid 6-color palette, native custom color picker, recoloring for cards/sticky notes/links/groups, duplicate, delete), zero-footprint hover action buttons, intelligent handle fade-in, and `EdgeToolbar` (in-place label editing, explicit arrow direction, solid/dashed/dotted style, width, and color controls). Harmonized scoped typography eliminates large heading underlines and prevents unwanted scrollbars, ensuring identical 13.5px line-height in both preview and editing modes. Canvas also provides Undo/Redo, local draft restoration after reload, publish-time persistence with relaxed fallback recovery, dark-mode styling, and bilingual starter templates. The Canvas toolbar imports and exports `.canvas` files. The Footer `Import content` menu remains available for Word, PDF, Office, audio, URL, and Markdown workflows. Read-only sharing keeps pan and zoom while locking edits.
