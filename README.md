...
- **三軌筆記格式與建立入口 (Markdown / Block / Canvas)**：Footer 最左側的「＋ 新增」選單支援建立 [Markdown 筆記](/new/markdown)、[Block 筆記](/new/block) 或 [Canvas 畫布](/new/canvas)。建立後格式鎖定；Markdown 專注線性純文字排版，Block 提供 Notion 風格區塊操作，Canvas 則提供 2D 空間卡片與關係線。
- **🎨 Canvas v2 畫布整體架構 (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。全面升級為 Canvas v2 架構，借鑑成熟開源工具專案 **Ameliorate**（`0cacee5577438979b651dd808793c4cbd13864ee`，MIT License，版權致謝於 `THIRD_PARTY_NOTICES.md`）：
  - **純淨空白起始與 100% 格式保真**：新建畫布以純淨空白起始 (`{ nodes: [], edges: [] }`)，不再強制插入範本卡片。底層相容 JSON Canvas 規範（`text`、`file`、`link`、`group` 節點與 `edges`），透過 `model/jsonCanvasAdapter.mjs` 完整保留未識別之 Root Metadata、Node/Edge 自訂屬性與 `david888` 擴充，確保 round-trip 100% 無損。
  ...
  - **順暢卡片拖曳與多方向自由縮放**：卡片 Header 即為移動拖曳把手；點選卡片立即喚起 8 方向縮放控制把手，縮放時即時同步更新尺寸，並與建立、移動、刪除、換色等操作一併納入 Zustand 交易歷史，支援 Cmd+Z / Cmd+Shift+Z 復原與重做。
  - **模組化節點系統 (`FlowNode`)**：外殼具備四邊 Loose handles、縮放邊框與選取狀態，專門子組件負責 `MarkdownNode`、`StickyNode`、`WikiNode`、`LinkNode` 與 `GroupNode`；選取時呈現浮動動作工具列 `NodeToolbar`，並透過 `computeContrastTheme` 依底色自動計算 WCAG 高對比文字與次要色彩。
  - **連線門戶與樣式面板 (`CanvasEdge` &amp; `EdgeToolbar`)**：Smoothstep 貝茲曲線與 20px 隱形感應軌道，支援標籤文字原地編輯；選取時透過 React Flow 原生 `EdgeToolbar` 門戶彈出連線面板，自動避讓視口邊界與窄螢幕 (320px) 碰撞，提供箭頭方向（無、起點、終點、雙向）、線條樣式（實線、虛線、點線）、粗細與顏色即時切換。
  - **導航控件、視口中心建立與檔案操作**：內建 React Flow `Controls`（縮放、適應視圖、鎖定）與右下角 `MiniMap` 小地圖；置底工具列 (`MainToolbar`) 新增卡片自動定位於當前視口中心並具備碰撞位移防重疊；檔案選單支援 `.canvas` 匯入、匯出與可隨時 Undo 復原的「清空畫布 (`clearDocument`)」。
  - **平滑回退開關與獨立隔離樣式**：支援網址參數 `?v=legacy` 或 `localStorage.getItem('CANVAS_EDITOR_VERSION') === 'legacy'` 隨時無縫降級回退至舊版；樣式獨立於 `static/js/canvas-v2/canvas-v2.css` 與語意化 `tokens.css`，零污染核心筆記佈局，支援 20 款主題與深淺色模式，唯讀分享模式鎖定座標防誤觸。
...
- **PWA 獨立應用、Web Share Target、跨裝置檔案關聯、衝突比對與全功能離線工作站 (PWA Offline Workstation)**：
  ...
  - **Local-First 本機離線錄音、Dynamic Island 懸浮提醒膠囊、連線 ASR 轉錄與發布延遲 S3 雲端同步 (Local-First Offline Audio Recording, Dynamic Island Recording HUD, Online ASR & Deferred S3 Upload)**：
    ...
    - **本機即時播放**：錄音完成音檔立即存入客戶端 **IndexedDB (`CloudNotepadOfflineDB` -> `audios` store)**，游標處插入本機播放器 `<audio controls data-offline-audio-id="rec_..." src="blob:..."></audio>`，提供 0ms 本機即時播放。
...
- **Three Fixed Note Formats &amp; Creation Menu (Markdown / Block / Canvas)**: The leftmost Footer `+ New` menu creates a [Markdown note](/new/markdown), a [Block note](/new/block), or a [Canvas](/new/canvas). The format is fixed after creation: Markdown retains its linear workflow, Block uses a Notion-like WYSIWYG editor, and Canvas provides 2D cards and relationship lines.
- **🎨 Canvas v2 Architecture (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. Fully upgraded to Canvas v2 based on the open-source **Ameliorate** project (`0cacee5577438979b651dd808793c4cbd13864ee`, MIT License, credited in `THIRD_PARTY_NOTICES.md`):
  - **Clean Blank Canvas & 100% Round-Trip Fidelity**: Blank canvases initialize completely clean (`{ nodes: [], edges: [] }`) without intrusive template cards. Strictly compatible with JSON Canvas 1.0 specifications (`text`, `file`, `link`, `group` nodes and `edges`) via `model/jsonCanvasAdapter.mjs`, preserving unhandled root metadata, custom node/edge properties, and `david888` extensions without data loss.
  ...
  - **Reliable Card Dragging & Multidirectional Resizing**: Card headers serve as drag handles for moving anywhere on the 2D canvas; clicking any card activates 8-direction resize handles. Resizing updates dimensions in real-time and joins node creation, dragging, deletion, and restyling in Zustand's transactional history for seamless Cmd+Z / Cmd+Shift+Z Undo and Redo.
  - **Modular Node System (`FlowNode`)**: Shared outer shell with four-sided Loose handles, resize boundaries, and selection indicators, with dedicated content components for `MarkdownNode`, `StickyNode`, `WikiNode`, `LinkNode`, and `GroupNode`. Features contextual `NodeToolbar` and automatic WCAG contrast calculation (`computeContrastTheme`) based on background luminance.
  - **EdgeToolbar Portal & Style Popovers (`CanvasEdge` &amp; `EdgeToolbar`)**: Smoothstep curves with 20px hit detection and inline label editing. Mounted via React Flow's native `EdgeToolbar` portal with viewport clamping and narrow-screen (320px) collision avoidance, offering quick controls for arrow direction (none, start, end, bidirectional), stroke styles (solid, dashed, dotted), line weights, and palette colors.
  - **Navigation Controls, Viewport-Center Card Creation & File Actions**: Includes React Flow `Controls` (zoom, fit-view, lock) and a bottom-right `MiniMap`; bottom `MainToolbar` creates cards centered in the current viewport with anti-overlap offsets; File menu supports `.canvas` import, export, and fully undoable Clear Canvas (`clearDocument`).
  - **Runtime Legacy Fallback & Isolated Stylesheet**: Seamless fallback to the legacy canvas via `?v=legacy` query parameter or `CANVAS_EDITOR_VERSION=legacy` setting. Styles are completely isolated in `static/js/canvas-v2/canvas-v2.css` and `tokens.css`, ensuring zero style bleed, dark/light theme support, and read-only gesture locking in share views.
...
- **PWA Application, Web Share Target, Multi-Device File Handling, Conflict Diff & Offline Workstation**:
  ...
  - **Local-First Offline Audio Recording, Dynamic Island Recording HUD, Online ASR & Deferred S3 Upload**:
    ...
    - **Instant Local Playback**: Instant microphone recording directly to client-side **IndexedDB (`CloudNotepadOfflineDB` -> `audios` store)**; inserts `<audio controls data-offline-audio-id="rec_..." src="blob:..."></audio>` for 0ms local playback.
