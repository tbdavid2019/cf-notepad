...
- **三軌筆記格式與建立入口 (Markdown / Block / Canvas)**：Footer 最左側的「＋ 新增」選單支援建立 [Markdown 筆記](/new/markdown)、[Block 筆記](/new/block) 或 [無限畫布 (Canvas)](/new/canvas)。建立後格式鎖定，Markdown 專注傳統線性純文字排版，Block 提供 Notion 風格區塊操作，Canvas 則提供 2D 空間視覺化卡片與連線。
- **🎨 2D 無限畫布與卡片連線 (Infinite Canvas with JSON Canvas & React Flow)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。採用開放標準 JSON Canvas (`jsoncanvas.org`)，原生相容 Obsidian Canvas（`.canvas` 檔案），支援雙向匯出與匯入；內建 Markdown 筆記卡片（雙擊原地編輯、失焦即時排版）、5 色彩靈感便籤（Sticky Notes）與 Wiki 筆記引用卡片；拖曳節點四邊錨點即可拉出平滑箭頭連線；編輯模式具備完整工具列與即時自動存檔，唯讀分享模式鎖定座標防手滑誤觸，同時保留自由平移（Pan）與縮放（Zoom）。
...
- **Three Fixed Note Formats &amp; Creation Menu (Markdown / Block / Canvas)**: The leftmost Footer `+ New` menu creates either a [Markdown note](/new/markdown), a [Block note](/new/block), or an [Infinite Canvas note](/new/canvas). The format is fixed after creation: Markdown retains its plain-text linear workflow, Block uses a single-column Notion-like WYSIWYG editor, and Canvas delivers a 2D spatial visualization whiteboard with interconnected cards.
- **🎨 2D Infinite Canvas with JSON Canvas &amp; React Flow**: A dedicated third native format (`editorFormat: 'canvas'`) designed for visual thinking, mind-mapping, and Zettelkasten card-linking. Built on the open standard JSON Canvas (`jsoncanvas.org`), offering 100% native compatibility with Obsidian Canvas (`.canvas` files) with two-way import and export; features Markdown note cards (double-click to edit in-place, blur for instant rich rendering), 5-color inspiration Sticky Notes, and live Wiki Note reference cards; drag connector handles from any node border to form smooth animated arrow edges. Edit mode includes an intuitive glassmorphic floating toolbar and real-time autosave, while read-only / share mode locks node positions to prevent accidental touch shifts while preserving free pan and zoom navigation.
