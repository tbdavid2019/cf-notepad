...
- **🎨 Canvas 畫布與卡片連線 (Canvas with JSON Canvas & React Flow)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。資料以 JSON Canvas 1.0 核心儲存，支援標準 text、file、link、group 節點與 edges；便籤外觀以可忽略的 `david888` extension 保存，舊版便籤資料會自動遷移。Canvas 工具列可匯入／匯出 `.canvas` 檔案；內建 Markdown 卡片、彩色便籤、Wiki 引用、外部連結與群組節點。四邊連接點均可建立高對比箭頭關係線，重複連線會提示。Footer 的「匯入內容」仍保留 Word、PDF、Office、音訊、網址與 Markdown 等既有功能；在 Canvas 情境會依既有匯入流程建立或處理 Markdown 筆記，不會直接改寫 Canvas JSON。唯讀分享模式鎖定座標，同時保留平移（Pan）與縮放（Zoom）。
...
- **🎨 Canvas with JSON Canvas &amp; React Flow**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. It persists the JSON Canvas 1.0 core—standard text, file, link, group, and edge records—while DAVID888 sticky-note presentation uses an ignorable `david888` extension and legacy sticky notes migrate automatically. The Canvas toolbar imports and exports `.canvas` files; it supports Markdown cards, colored sticky notes, Wiki references, web links, and groups. All four handles create high-contrast arrow connections and duplicate connections report feedback. The Footer `Import content` menu remains available for Word, PDF, Office, audio, URL, and Markdown workflows; from Canvas it follows the existing Markdown-note import flow and does not overwrite Canvas JSON. Read-only sharing keeps pan and zoom while locking edits.
