...
- **PWA 獨立應用、Web Share Target、跨裝置檔案關聯、衝突比對與全功能離線工作站 (PWA Offline Workstation)**：
  ...
  - **Local-First 本機離線錄音、即時 Recording HUD 提醒、連線 ASR 轉錄與發布延遲 S3 雲端同步 (Local-First Offline Audio Recording, Live Recording HUD, Online ASR & Deferred S3 Upload)**：
    - **即時錄音提醒懸浮膠囊 (Live Recording HUD)**：錄音時頂部自動展開懸浮膠囊，提供呼吸紅點、聲波動畫、`MM:SS` 即時計時器，並內建「⏸️ 暫停 / ▶️ 繼續」、「⏹️ 完成並插入」與「✕ 取消放棄」全功能直覺按鈕。
...
- **PWA Application, Web Share Target, Multi-Device File Handling, Conflict Diff & Offline Workstation**:
  ...
  - **Local-First Offline Audio Recording, Live Recording HUD, Online ASR & Deferred S3 Upload**:
    - **Live Floating Recording HUD**: Unfolds a responsive glassmorphism HUD pill at the top of the viewport with pulsing live red dot, audio waveform animation, `MM:SS` live duration timer, and one-click "⏸️ Pause / ▶️ Resume", "⏹️ Done & Insert", and "✕ Cancel & Discard" controls.
