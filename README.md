...
- **🎨 Canvas v2 Architecture (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. Fully upgraded to Canvas v2 with deep parity to the open-source **Ameliorate** project (`0cacee5577438979b651dd808793c4cbd13864ee`, MIT License, credited in `THIRD_PARTY_NOTICES.md`):
  ...
  - **Zero Auto-Draft Overwrite, Clean Blank Slate & Public Share Auto-Detection**: Completely removed asynchronous IndexedDB draft restoration—notes strictly load the server-side document as the single source of truth. Blank canvases initialize completely clean (`{ nodes: [], edges: [] }`) without intrusive template cards. Public Share (`/share/:shareId`) automatically detects canvas JSON content even when legacy metadata is missing, mounting the read-only canvas with version-busted assets (`?v=2.2`). Strictly compatible with JSON Canvas 1.0 specifications (`text`, `file`, `link`, `group` nodes and `edges`) via `model/jsonCanvasAdapter.mjs`, preserving unhandled root metadata, custom node/edge properties, and `david888` extensions without data loss.
