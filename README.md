...
- **🎨 Canvas v2 畫布整體架構 (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**：專為空間視覺化思考與卡片盒筆記（Zettelkasten）打造的第三種原生模式 (`editorFormat: 'canvas'`)。全面升級為 Canvas v2 架構，深度對齊開源專案 **Ameliorate**（`0cacee5577438979b651dd808793c4cbd13864ee`，MIT License，版權致謝於 `THIRD_PARTY_NOTICES.md`）：
  - **Ameliorate 經典卡片構造與思考節點**：卡片左上角內嵌飽滿色彩的類型標籤（`🧩 Problem`、`🌱 Benefit`、`💡 Solution`、`🔀 Cause`、`📋 Criterion`、`⚠️ Detriment`、`❓ Question`、`📄 Note`），左上圓角貼合卡片邊框、右下圓角收折，徹底消除傳統視窗水平頂欄與多餘留白。卡片底色採淡雅粉彩色塊搭配 2px 原生彩色邊框與自然陰影，文字採精緻等寬字型排版，新建卡片預設為 `new node`，點擊即可無縫行內編輯。
  - **貝茲曲線與 causes 關係線 (`CanvasEdge` &amp; `EdgeToolbar`)**：連線路徑採用平滑貝茲曲線 (`getBezierPath`)，預設關係文字自動標註為 `causes`，線上掛載鏤空三角箭頭 (`▷`)；選取時透過 React Flow 原生 `EdgeToolbar` 門戶彈出連線面板，自動避讓視口邊界與窄螢幕 (320px) 碰撞，提供箭頭方向、線條樣式、粗細與顏色即時切換。
  - **右上角懸浮操作膠囊 (`TopFloatingBar`)**：畫布右上角提供玻璃擬態快捷膠囊，支援 `文A`（中英切換）、`🔊`（Web Speech 語音朗讀卡片內容）、`📖`（書本閱讀模式）。
  ...
  - **順暢卡片拖曳與多方向自由縮放**：卡片本身即為移動拖曳把手；點選卡片立即喚起 8 方向縮放控制把手，縮放時即時同步更新尺寸，並與建立、移動、刪除、換色等操作一併納入 Zustand 交易歷史，支援 Cmd+Z / Cmd+Shift+Z 復原與重做。
...
- **🎨 Canvas v2 Architecture (Canvas v2 with Ameliorate Architecture &amp; JSON Canvas)**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. Fully upgraded to Canvas v2 with deep parity to the open-source **Ameliorate** project (`0cacee5577438979b651dd808793c4cbd13864ee`, MIT License, credited in `THIRD_PARTY_NOTICES.md`):
  - **Ameliorate Thought Nodes & Corner Type Badges**: Cards feature Ameliorate's signature corner badge with vivid type tags (`🧩 Problem`, `🌱 Benefit`, `💡 Solution`, `🔀 Cause`, `📋 Criterion`, `⚠️ Detriment`, `❓ Question`, `📄 Note`), rounded to the top-left edge without window title bars. Card backgrounds use pastel tint blocks with 2px matching borders and subtle shadows; text uses clean monospace typography defaulting to `new node` with seamless click-to-edit inline inputs.
  - **Bézier Curves & causes Relation Edges (`CanvasEdge` &amp; `EdgeToolbar`)**: Edge connections render as smooth Bézier curves (`getBezierPath`) with the default `causes` relation label and hollow triangle arrows (`▷`); selected edges reveal React Flow's native `EdgeToolbar` portal for arrow directions, line styles, weights, and color adjustments.
  - **Top-Right Floating Action Pill (`TopFloatingBar`)**: A glassmorphic quick-action pill in the top-right corner provides `文A` (bilingual language switch), `🔊` (Web Speech card read-aloud), and `📖` (Book / Reader mode).
  ...
  - **Reliable Card Dragging & Multidirectional Resizing**: Card bodies serve as drag handles for moving anywhere on the 2D canvas; clicking any card activates 8-direction resize handles. Resizing updates dimensions in real-time and joins node creation, dragging, deletion, and restyling in Zustand's transactional history for seamless Cmd+Z / Cmd+Shift+Z Undo and Redo.
