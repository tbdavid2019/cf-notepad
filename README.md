...
- **🎨 Canvas with JSON Canvas &amp; React Flow**: A third native format (`editorFormat: 'canvas'`) for visual thinking, mind-mapping, and Zettelkasten card linking. It persists the JSON Canvas 1.0 core—standard text, file, link, group, and edge records—with interactive `NodeToolbar` (6-color Obsidian palette, sticky recoloring, card duplicate, delete) and `EdgeToolbar` (in-place label editing, 4-way arrow direction toggle, solid/dashed/dotted line styles, line width, and color palette). The Canvas toolbar imports and exports `.canvas` files; it supports Markdown cards, colored sticky notes, Wiki references, web links, and groups. The Footer `Import content` menu remains available for Word, PDF, Office, audio, URL, and Markdown workflows. Read-only sharing keeps pan and zoom while locking edits.
